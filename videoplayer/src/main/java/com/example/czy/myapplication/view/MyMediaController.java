...
//    private MediaPlayerControl player;
    ...
    {
        Field mProgress = null;
        try
        {
            mProgress = MediaController.class.getDeclaredField("mProgress");
            mProgress.setAccessible(true); // 抑制Java对修饰符的检查
            if (mProgress != null)
            {
                SeekBar seekBar = (SeekBar)mProgress.get(this);
    ...
    public void onFinishSeekBar()
    {
        try
        {
            SeekBar mProgress = (SeekBar)getParentReflect("mProgress");
            if (mProgress == null)
            {
                return;
            }

            boolean mInstantSeeking = (boolean)getParentReflect("mInstantSeeking");
            MediaPlayerControl mPlayer = (MediaPlayerControl)getParentReflect("mPlayer");
            long mDuration = (long)getParentReflect("mDuration");
            OutlineTextView mInfoView = (OutlineTextView)getParentReflect("mInfoView");
            int sDefaultTimeout = (int)getParentReflect("sDefaultTimeout");
            Handler mHandler = (Handler)getParentReflect("mHandler");
            AudioManager mAM = (AudioManager)getParentReflect("mAM");
            boolean mDragging = (boolean)getParentReflect("mDragging");
            ...
            setParentReflect("mPlayer", mPlayer);
            setParentReflect("mInfoView",mInfoView);
            setParentReflect("mHandler",mHandler);
            setParentReflect("mAM",mAM);
            setParentReflect("mDragging",mDragging);
        } catch (Exception e)
        {
            e.printStackTrace();
        }
    ...
    public String setSeekBarChange(int progress)
    {
        ...
        try
        {
            ...
            MediaPlayerControl mPlayer = (MediaPlayerControl)getParentReflect("mPlayer");
            OutlineTextView mInfoView = (OutlineTextView)getParentReflect("mInfoView");
            TextView mCurrentTime = (TextView)getParentReflect("mCurrentTime");

            if (mProgress == null)
                return "";
            mProgress.setProgress(progress);
            long newposition = (mDuration * progress) / 1000;
            time = StringUtils.generateTime(newposition);
            if (mInstantSeeking)
            ...
            setParentReflect("mProgress", mProgress);
            setParentReflect("mPlayer", mPlayer);
            setParentReflect("mInfoView",mInfoView);
            setParentReflect("mCurrentTime",mCurrentTime);
        } catch (Exception e)
        {
            e.printStackTrace();
        }
    ...
    private Object getParentReflect(String name) throws NoSuchFieldException, IllegalAccessException
    {
        Field field = MediaController.class.getDeclaredField(name);
        field.setAccessible(true); // 抑制Java对修饰符的检查
        return field.get(this);
    }

    private void setParentReflect(String name,Object value) throws NoSuchFieldException, IllegalAccessException
    {
        Field field = MediaController.class.getDeclaredField(name);
        field.setAccessible(true); // 抑制Java对修饰符的检查
